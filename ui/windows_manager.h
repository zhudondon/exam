  ...
  */
    ...
    public:
        ...
        */
        ...
        template<typename WindowType>
        static WindowType* SingletonShowNoLayer(const std::wstring& window_id, bool show_window=true)
        {
            WindowType *window = (WindowType*)(WindowsManager::GetInstance()->GetWindow(WindowType::kClassName, window_id));
            if (!window)
            {
                window = new WindowType;
                window->Create(NULL, WindowType::kClassName, WS_OVERLAPPEDWINDOW & ~WS_MAXIMIZEBOX, 0, false);
                window->CenterWindow();
                window->ShowWindow(show_window);
            }
            else
            {
                window->ActiveWindow();
