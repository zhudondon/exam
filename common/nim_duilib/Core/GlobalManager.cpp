...
std::wstring GlobalManager::m_pStrLangPath;
...
void GlobalManager::Startup(const std::wstring& strResourcePath, const CreateControlCallback& callback, bool bAdaptDpi, const std::wstring& theme, const std::wstring& language)
{
	...
	DWORD len = ::GetModuleFileName(NULL, directory, MAX_PATH);
	if (len && len <= MAX_PATH)
	{
		WCHAR* pos = _tcsrchr(directory, '\\');
		if (pos)
		{
			*pos = '\0';
			m_pStrLangPath = directory;
			m_pStrLangPath += L"\\";
		}
	}
	// 加载多语言文件，如果使用了资源压缩包则从内存中加载语言文件
	if (g_hzip) {
		HGLOBAL hGlobal = GetData(m_pStrLangPath + language + L"\\gdstrings.ini");
		if (hGlobal) {
		...
		MutiLanSupport::GetInstance()->LoadStringTable(m_pStrLangPath + language + L"\\gdstrings.ini");
	}
