...
namespace nim_cef
{
    ...
    typedef std::function<void(bool ret_success, const std::string& result)> ReportResultFunction;

    typedef std::function<void(const std::string& result)> CallJsFunctionCallback;
    typedef std::function<void(const std::wstring& fun_name, const CefRefPtr<CefListValue>& argList, int js_callback_id, ReportResultFunction callback)> CppFunction;
...
// in render process
    public:
        ...
        bool CallCppFunction(const CefString& function_name, const CefRefPtr<CefListValue> params, CefRefPtr<CefV8Value> callback);
         ...
         * param[in] ret_success		是否成功，对应回调函数第一个参数
         ...
         */
        bool ExecuteJSCallbackFunc(int js_callback_id, bool ret_success, const CefString& json_result);
...
// in browser process
    public:
        ...
        bool ExecuteCppFunc(const CefString& function_name, const CefRefPtr<CefListValue>& argList, int js_callback_id, CefRefPtr<CefBrowser> browser);
    private:
