...
namespace nim_cef {
    ...
    bool CefJSBridge::CallCppFunction(const CefString& function_name, const CefRefPtr<CefListValue> params, CefRefPtr<CefV8Value> callback)
    {
        auto it = render_callback_.find(js_callback_id_);
        if (it == render_callback_.cend())
        {
            int callback_id = js_callback_id_;
            ...
            message->GetArgumentList()->SetList(1, params->Copy());
            if (!callback.get())			//如果没有 callback,则不加入到列表
                callback_id = 0;
            else
                render_callback_.emplace(js_callback_id_++, std::make_pair(context, callback));

            message->GetArgumentList()->SetInt(2, callback_id);
...
//}
    ...
    bool CefJSBridge::ExecuteJSCallbackFunc(int js_callback_id, bool ret_success, const CefString& json_result)
    {
        ...
        if (it != render_callback_.cend())
        {
            ...
            if (context.get() && callback.get())
            {
                ...
                // 第一个参数标记函数执行结果是否成功
                arguments.push_back(CefV8Value::CreateBool(ret_success));
    ...
    bool CefJSBridge::ExecuteCppFunc(const CefString& function_name, const CefString& params, int js_callback_id, CefRefPtr<CefBrowser> browser)
    {
        ...
        //	auto function = it->second;
        //	Post2UI([=]() {
        //		function(params, [=](bool ret_success, const std::string& json_result) {
        //			// 测试代码，需要封装到管理器中
        //			args->SetInt(0, js_callback_id);
        //			args->SetBool(1, ret_success);
    ...
    bool CefJSBridge::ExecuteCppFunc(const CefString& function_name, const CefRefPtr<CefListValue>& argList, int js_callback_id, CefRefPtr<CefBrowser> browser)
    {
        CefRefPtr<CefProcessMessage> message = CefProcessMessage::Create(kExecuteJsCallbackMessage);
        CefRefPtr<CefListValue> args = message->GetArgumentList();
        ...
        {
            auto function = it->second;
            CefRefPtr<CefListValue> newArg = argList->Copy();
            Post2UI([=]() {
                int sz = newArg->GetSize();
                int x = newArg->GetInt(0);
                int y = newArg->GetInt(1);
                function(function_name, newArg, js_callback_id, [=](bool ret_success, const std::string& json_result) {
                    // 测试代码，需要封装到管理器中
                    args->SetInt(0, js_callback_id);
                    args->SetBool(1, ret_success);
                    args->SetString(2, json_result);
        ...
        if (it != browser_registered_function_.cend())
        {
            auto function = it->second;
            Post2UI([=]() {
                function(function_name, argList, js_callback_id, [=](bool ret_success, const std::string& json_result) {
                    // 测试代码，需要封装到管理器中
                    args->SetInt(0, js_callback_id);
                    args->SetBool(1, ret_success);
                    args->SetString(2, json_result);
