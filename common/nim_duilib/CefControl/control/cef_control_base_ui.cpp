...
namespace ui
{
    ...
    CefRefPtr<CefBrowser> CefControlBase::GetCefBrowser() const
    {
        if (browser_handler_.get() && browser_handler_->GetBrowser().get())
            return browser_handler_->GetBrowser();

        CefRefPtr<CefBrowser> p;
        return p;
    }
    ...
    bool CefControlBase::CallJSFunction(const std::wstring& js_function_name, const std::wstring& params, nim_cef::CallJsFunctionCallback callback, const std::wstring& frame_name /*= L""*/)
    {
        if (browser_handler_.get() && browser_handler_->GetBrowser().get() && js_bridge_.get())
        {
            ...
            {
                nbase::QLOG_ERR(L"Failed to call JavaScript function {0}") << js_function_name;
                return false;
            }
    ...
    bool CefControlBase::CallJSDirect(std::string& js)
    {
        bool ret = false;
        if (browser_handler_.get() && browser_handler_->GetBrowser().get())
        {
    ...
    bool CefControlBase::CloseBrowser()
    {
        bool bCancel = false;

        if (browser_handler_ && !browser_handler_->GetBrowser().get() && is_closing_) {
    ...
    bool CefControlBase::AttachDevTools(Control* view)
    {
