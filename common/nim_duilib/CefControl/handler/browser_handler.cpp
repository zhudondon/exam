...
namespace nim_cef
{
    ...
    bool BrowserHandler::OnProcessMessageReceived(CefRefPtr<CefBrowser> browser, CefProcessId source_process, CefRefPtr<CefProcessMessage> message)
    {
        ...
        {
            //0:funcation name, 1:arglist, 2,callBackId
            CefRefPtr<CefListValue> argList;
            CefString param;
            CefString fun_name	= message->GetArgumentList()->GetString(0);
            if (message->GetArgumentList()->GetType(1) == VTYPE_LIST)
            {
                argList = message->GetArgumentList()->GetList(1);
                int sz = argList->GetSize();
                int x = argList->GetInt(0);
                int y = argList->GetInt(1);
                y = y;
            }
            else
                param = message->GetArgumentList()->GetString(1);
            int js_callback_id	= message->GetArgumentList()->GetInt(2);

            if (handle_delegate_)
            {
                if(argList.get())
                    handle_delegate_->OnExecuteCppFunc(fun_name, argList, js_callback_id, browser);
                //else
...
// CefDisplayHandler methods
    ...
    {
        if (handle_delegate_)
            nbase::ThreadManager::PostTask(kThreadUI, nbase::Bind(&HandlerDelegate::OnFullscreenModeChange, handle_delegate_, browser, fullscreen));
    }
...
// CefLoadHandler methods
    ...
    void BrowserHandler::OnLoadStart(CefRefPtr<CefBrowser> browser, CefRefPtr<CefFrame> frame, TransitionType transition_type)
    {
        ...
        if (handle_delegate_)
            nbase::ThreadManager::PostTask(kThreadUI, nbase::Bind(&HandlerDelegate::OnLoadStart, handle_delegate_, browser, frame, transition_type));
    }
...
//#endif
        ...
        //suppress_message=false,且返回值也为false,则不会屏蔽js对话框
